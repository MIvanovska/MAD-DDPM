from math import pi
from torch import nn, addcmul, tensor, arange, cat, randn
...
class Denoiser(nn.Module):
    ...
    def add_noise(self, input, noise, sigma, **kwargs):
        c_skip, c_out, c_in = [utils.append_dims(x, input.ndim) for x in self.get_scalings(sigma)]
        noised_input = input + noise * utils.append_dims(sigma, input.ndim)
        return noised_input * c_in
...
class AdaGN(ConditionedModule):
    ...
    def forward(self, input, cond):
        ...
        return addcmul(utils.append_dims(bias, input.ndim), input, utils.append_dims(weight, input.ndim) + 1)
...
class Downsample2d(nn.Module):
    def __init__(self, kernel='linear', pad_mode='reflect'):
        ...
        kernel_1d = tensor([_kernels[kernel]])
    ...
    def forward(self, x):
        ...
        indices = arange(x.shape[1], device=x.device)
...
class Upsample2d(nn.Module):
    def __init__(self, kernel='linear', pad_mode='reflect'):
        super().__init__()
        self.pad_mode = pad_mode
        kernel_1d = tensor([_kernels[kernel]]) * 2
        self.pad = kernel_1d.shape[1] // 2 - 1
        self.register_buffer('kernel', kernel_1d.T @ kernel_1d)
...
class FourierFeatures(nn.Module):
    def __init__(self, in_features, out_features, std=1.):
        ...
        self.register_buffer('weight', randn([out_features // 2, in_features]) * std)

    def forward(self, input):
        f = 2 * pi * input @ self.weight.T
        return cat([f.cos(), f.sin()], dim=-1)
